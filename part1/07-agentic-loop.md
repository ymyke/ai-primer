...
A concrete example: ask an agent to "prepare a competitive analysis." With only web search, it can research and summarize. Add shell access, and it can write and run code to scrape data, produce charts, and compile a formatted report. Add integrations — email, calendar, cloud storage — and it can send the finished report to your team and schedule a follow-up meeting. Same loop, same model, radically different outcomes.

Products like Claude Code, Cursor, or Claude's computer use feature are, at their core, an agentic loop with shell access. Dedicated tool integrations — Google Calendar, Gmail, Slack, CRM — are a convenience layer: they make specific tasks more reliable, but the underlying mechanism is always the same loop. MN still not sure if this para is really helpful. is it? panel?
...
When someone says "we're deploying an agent," the first question to ask is: *what tools does it have?* That tells you more about what it can do — and what can go wrong — than any other detail. MN also here: is this really valuable or mostly blabbla? is this how we should end this section? panel?

MN should we mention openclaw here (or elsewhere?) what are the interesting things to say about openclaw? is it the same as claude code, cursor etc.? panel debate and lmk!